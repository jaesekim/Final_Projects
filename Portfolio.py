...
for i in range(len(df_top3)):
    ...
    plt.scatter(portfolio_std(1, input_item[1], top3_std, top3_corr), 
                portfolio_return(1, input_item[0], 
                top3_return),c='c', marker='X', s=150)
    plt.xlabel("std(σ)")
    ...
    st.write(f"{top3_name}의 보유 비중: {np.round(1 - mvp_weight, 2)}")